...
export const mockLogin = async (req, res) => {
  ...
  try {
    const { role = 'buyer' } = req.body;
    let user = await User.findOne({ email: `mock${role}@example.com` });
    if (!user) {
      user = await User.create({
        googleId: `mock_google_id_${role}`,
    ...
    return res.status(500).json({ message: 'Mock login failed' });
  }
};
